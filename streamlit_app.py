    ...
    while True:
        try:
            ...
            if is_full_scan:
                ...
                if elapsed > 3: targets += [{"ip": ip, "src": "💎 冷门挖掘"} for ip in safe_read_json(NICHE_FILE, [])]
...
if data:
    ...
    # 修复导致 KeyError 的标题逻辑
    mode_name = data.get('mode', '默认策略')
    st.subheader(f"🧬 基因库排行 (策略: {mode_name})")
    
    # 数据表格格式化 (复原 1000036590.jpg 和 1000036593.jpg)
    df = pd.DataFrame(data['table'])
    
    def format_icon(src):
        if "本地种子" in src: return "⚡ 本地种子"
        if "历史" in src: return "📂 历史优选"
        if "爬虫" in src: return "🕷️ 爬虫补位"
        return f"💎 {src}"

    df['分类标记'] = df['src'].apply(format_icon)
    df['地理位置'] = df['cc'] + " " + df['country']

    st.dataframe(
        df,
        column_order=("score", "分类标记", "ip", "地理位置", "avg", "speed", "last_test"),
        column_config={
