...
def evolution_engine():
    ...
    while True:
        try:
            ...
            if first_scan_done:
                if db:
                    sorted_db = sorted(db.items(), key=lambda x: x[1].get('score', 0), reverse=True)[:80]
            ...
            workers = 8 if not first_scan_done else cfg["max_workers"]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                futures = [executor.submit(test_ip, t) for t in unique_targets[:100]]
