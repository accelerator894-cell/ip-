...
def get_geo_info(ip: str, timeout=3.5) -> dict:
    now = time.time()
...
with st.sidebar:
    ...
    with st.expander("高级设置"):
        ...
        max_workers = st.slider("最大并发", 10, 60, cfg["max_workers"], step=5)
...
if not data or not data.get("winner"):
    ...
    st.info("引擎启动中... 预计5~15秒初次扫描完成（优先本地历史IP）")
    time.sleep(3)
    st.rerun()
